...
// GROVE START IF STATEMENT
  if(DHT.humidity>=50){ //if the humidity % is equal or above the setpoint 51% the atomizer turn ON (HIGH)
      ...
      Serial.println("Posting Atomizer OFF" +  String(motorValue, 1) + " to ThingSpeak");
      ...
      Serial.println("Posting Atomizer ON" + String(motorValue, 0) +  " to ThingSpeak");
      ...
      digitalWrite(motorPin, LOW);
      Serial.print("ATOMIZER OFF \n MOTOR OFF \n");
      Serial.println("Posting Atomizer OFF " +  String(motorValue, 0) +  " to ThingSpeak");
      //ThingSpeak.setField(6, String(motorValue= 0));
    }
// GROVE END
